...
def data_load(data_path:str) ->pd.DataFrame:
    ...
    try:
        ...
        return df
    except pd.errors.ParserError as e:# this error happend when pandas not read file properly 
...
def drop_unwanted_columne(df:pd.DataFrame) -> pd.DataFrame:
    ''' this is going to drop the unwanted columne of the data '''
    try:
        df=df.drop(columns=['id'],errors='ignore')
        logger.debug(f'the unwanted columne are drop sucessfully{df.shape}')
        return df
    except Exception as e:
...
def split_data(df:pd.DataFrame,test_size:float)->tuple:
    ...
    try:
        ...
        logger.info(f'data is successfully splited in shape of {train_df.shape} and {test_df.shape}')
...
def save(train_df:pd.DataFrame,test_df:pd.DataFrame,output_path:str) ->None:
    ...
    try:
        ...
        logger.info(f'the data in save in the file of  with the shape{train_df.shape} and {test_df.shape}')
...
def main():
    ...
    try:
        data_path='https://raw.githubusercontent.com/Rahul117590/Data-for-spam_prediction/refs/heads/main/breast-cancer.csv'
        test_size=0.2
        df=data_load(data_path=data_path)
        df=drop_unwanted_columne(df)
        train_df,test_df=split_data(df,test_size)
        save(train_df,test_df,output_path='data')
    
    except Exception as e:
        logger.error(f'Unexpected error happend in form of {e}')
